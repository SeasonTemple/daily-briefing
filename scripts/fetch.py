#!/usr/bin/env python3
"""
Daily build: fetch 45 源 + 调 GH Models (mistral-large) 做中文摘要 + 评价 + 写 RSS + HTML
GH Actions 一条龙,user 不用动。
"""
import argparse
import os
import re
...
SRC_TIER = {
    ...
    '36Kr': 'T5', 'V2EX AI': 'T5', 'Latent Space': 'T4',
}
...
# ============== GitHub Models API ==============
# 用 mistral-large (or fallback) 做中文摘要
def summarize_batch(items, model='mistral-large-2407', max_items=20):
    """调 GH Models API, 批量做中文摘要 + 评价 (省时间)"""
    token = os.environ.get('GITHUB_TOKEN')
    if not token:
        logging.warning("No GITHUB_TOKEN, skip LLM")
...
def _summarize_one(title_en, src_label, token, model='mistral-large-2407'):
...
要求:
- title_zh: 20-40字中文标题,精炼
- summary: 60-80字中文摘要,讲清楚事件/研究
- take: 30-50字中文评价,**给具体观点**(对比公司/数字/场景/对手),**禁止**'提供了理论基础/值得关注'等套话

{{"title_zh":"...","summary":"...","take":"..."}}"""
    # 试多个模型(mistral 优先)
    for m in [model, 'Mistral-large', 'gpt-4o', 'gpt-4o-mini']:
        try:
            r = requests.post(
            ...
            if r.status_code == 200:
                ...
                logging.debug(f"  {m} status {r.status_code}: {r.text[:100]}")
        except Exception as ex:
...
def render_rss(items, profile, out_path):
    desc = f'每日 07:00 (Asia/Shanghai) 自动抓取 45 个信源,LLM 中文摘要 + 评价。GitHub Actions 一条龙,user 不用动。本期 {len(items)} 条,11+ 源混搭。'
    ...
    with open(out_path, 'w', encoding='utf-8') as f:
        ...
        f.write('    <ttl>5</ttl>\n')
        f.write('    <generator>daily-briefing/2.0 (GH Actions + mistral-large)</generator>\n\n')
...
def render_html(items, profile, out_path):
    ...
    gen = 'daily-briefing/2.0 (GH Actions + mistral-large)'
    desc = f'每日 07:00 (Asia/Shanghai) 自动抓取 45 个信源,LLM 中文摘要 + 评价。GitHub Actions 一条龙,user 不用动。本期 {total} 条。'
...
def main():
    ...
    p.add_argument('--out', default='rss/ai-daily.xml')
    p.add_argument('--html', default='digests/ai-daily.html')
    ...
    # 6. LLM 摘要
    if args.no_llm or not os.environ.get('GITHUB_TOKEN'):
        logging.info("LLM disabled, using fallback")
        for it in final:
            f = summarize_fallback(it)
            it.update(f)
    else:
        logging.info("Calling GH Models (mistral-large)...")
        results = summarize_batch(final, model='mistral-large-2407', max_items=args.max_items)
